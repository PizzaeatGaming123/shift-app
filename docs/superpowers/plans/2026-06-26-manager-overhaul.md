...
### Task 1.5: 「シフト管理」グループ名を「シフト確定」へリネーム
...
注: 現状のラベルは「シフト」。設計書で「シフト管理→シフト確定」と書いたが、コード上の現状ラベルは「シフト」なのでそれを「シフト確定」へ。

- [ ] **Step 2: テスト失敗確認**

Run: `cd frontend && npm test -- --run GlobalNav`

- [ ] **Step 3: NAV_GROUPS の `{ label: 'シフト', ... }` を `{ label: 'シフト確定', ... }` に変更**
...
```bash
git add -u
git commit -m "feat(manager): ナビのシフトグループ名を「シフト確定」へ"
```
