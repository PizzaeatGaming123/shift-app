...
beforeEach(() => {
  vi.restoreAllMocks();
  // 実機の今月を 2026-08 に固定し、既定の表示対象（翌月）を 9 月（30 日）に揃える。
  // setTimeout などは止めずに Date 系だけ差し替える。
...
it('店舗を切り替えるとシフト表のスタッフも切り替わる', async () => {
  const user = userEvent.setup();
  ...
  await user.selectOptions(screen.getByRole('combobox', { name: '店舗' }), '2');

  expect(await screen.findByText('新田一郎')).toBeInTheDocument();
  expect(screen.queryByText('田中太郎')).not.toBeInTheDocument();
});

it('表示チェックの状態をブラウザに保存する', async () => {
  const user = userEvent.setup();
  ...
  const notes = await screen.findByRole('checkbox', { name: '勤務メモ' });
  expect(notes).toBeChecked();

  await user.click(notes);
  expect(JSON.parse(localStorage.getItem('akiyume-shift-layers')!).showNotes).toBe(false);
  ...
  expect(await screen.findByRole('checkbox', { name: '勤務メモ' })).not.toBeChecked();
});
