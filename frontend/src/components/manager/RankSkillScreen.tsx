...
/** ランク階層（らくしふ初期値 S・A・B・C・D）。staff.rank の数値にマップする。 */
...
export function RankSkillScreen({ initialTab }: { initialTab: Tab }) {
  ...
  return (
    <div className="rk-ranks">
      <div className="rk-ranks__tabs" role="tablist">
      ...
      <div className="rk-ranks__search">
        <input
          type="search"
          aria-label={tab === 'rank' ? 'ランク名から検索' : 'スキル名から検索'}
          placeholder={tab === 'rank' ? 'ランク名から検索できます' : 'スキル名から検索できます'}
          value={query}
      ...
      <table className="rk-ranks__table">
        <thead>
          <tr>
